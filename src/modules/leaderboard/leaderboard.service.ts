  ...
  async getLeaderboard(startDate?: string, endDate?: string, leaderId?: string, email?: string, name?: string, leaderName?: string) {
    ...
    const rawData = await this.prisma.$queryRaw<any[]>`
      SELECT 
        ...
        SUM(CASE WHEN a."qualityScore" = true THEN 1 ELSE 0 END)::int AS "qualityPassed",
        SUM(CASE WHEN a."finalCheckScore" = true THEN 1 ELSE 0 END)::int AS "finalCheckPassed",
        SUM(CASE WHEN a."isOnTime" = true THEN 1 ELSE 0 END)::int AS "etaPassed"
      FROM "User" u
